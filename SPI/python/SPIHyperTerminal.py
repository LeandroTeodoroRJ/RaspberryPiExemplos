...
'''
Observações:
1- Falta implementação da rotina de recebimento de dados.
'''
...
janela.geometry('600x350+0+0')  # 'LARGURAxALTURA+DISTÂNCIA_ESQUERDA+DISTÂNCIA_DO_TOPO'
...
bt_abrir_porta = Button(janela, text='Abrir Porta SPI', width=30, command=bt_brir_porta_click)
bt_fechar_porta = Button(janela, text='Fechar Porta SPI', width=30, command=bt_fechar_porta_click)
...
tbox_enviado = Text(janela, wrap=WORD, width=70, height=3, yscrollcommand=rolbar1.set)
rolbar1.config(command=tbox_enviado.yview)   #Define que a barra de rolagem será anexada
                                                    #ao objeto Caixa_de_texto (text).
tbox_recebido = Text(janela, wrap=WORD, width=70, height=3, yscrollcommand=rolbar2.set)
